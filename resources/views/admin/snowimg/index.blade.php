...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Медиа уборка снега с крыш</h1>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <!-- /.card-header -->
                        <div class="card-body">
                            <a href="{{ route('postssnow.create') }}" class="btn btn-primary mb-3">Добавить фото</a>
                            @if (count($snow))
                                        ...
                                        @foreach($snow  as $post)
                                            <tr>
                                                <td style="width: 80px;">
                                                    ...
                                                    @else
                                                        <img src="{{ asset('snow/' . $post->path) }}" style="width: 100%; object-fit: cover" alt="">
                                                    @endif
                                                </td>
                                                <td>{{$result = date('Y-m-d H:i:s', strtotime($post->created_at) + 3 * 3600);}}</td>
                                                <td style="display: flex;">
                                                    <form action="{{ route('postssnow.destroy', $post->id) }}" method="post" class="float-left">
                                                        @csrf
                                                        @method('DELETE')
